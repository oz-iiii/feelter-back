					...
					>
						<div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
							...
							<div className="text-center md:text-left flex-1">
								...
								{user.selectedOttPlatforms &&
									user.selectedOttPlatforms.length > 0 && (
										<div className="mb-4">
											<p className="text-sm text-gray-500 mb-2">
												구독중인 OTT: {user.selectedOttPlatforms.length}개
											</p>
										...
										>
											...
											<div className="flex-1">
												...
												<p className="text-sm text-gray-400">
													{item.watchDate ? new Date(item.watchDate.replace(/\./g, '-')).getFullYear() : new Date().getFullYear()}{" "}
													• {Array.isArray(item.movieData?.genre) ? item.movieData.genre.join(", ") : item.movieData?.genre || "기타"}
												</p>
											</div>
										</div>
									))
										...
										>
											...
											<div className="flex-1">
												...
												<p className="text-sm text-gray-400">
													...
													• {Array.isArray(movie.genre) ? movie.genre.join(", ") : movie.genre || "기타"}
												</p>
											</div>
											<button
												className="text-red-500 hover:text-red-600 transition-colors"
												onClick={() => removeFromFavorites(movie.id as number)}
											>
												<svg
													className="w-5 h-5"
													fill="currentColor"
